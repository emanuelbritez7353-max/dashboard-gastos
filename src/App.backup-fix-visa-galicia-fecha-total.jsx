...
function detectarResumenPago(textoPDF, nombrePDF) {
  ...
  /*
    IMPORTANTE:
    Este regex solo acepta montos reales con decimales:
    72.558,37
    50630,00
    $ 50.630,00

    No acepta comprobantes ni números sueltos como 839910.
  */
  function obtenerMontos(texto) {
    const encontrados =
      String(texto).match(
        /(?:\$|ARS)?\s*-?(?:\d{1,3}(?:\.\d{3})+|\d+),\s*\d{2}/gi
      ) || [];
  ...
  function esLineaQueNoSirveParaTotal(linea) {
    const l = normalizarTexto(linea);
  ...
  /*
    1) TOTAL A PAGAR
    Buscamos primero una línea que contenga TOTAL A PAGAR.
    Si tiene monto, usamos ese.
    Si no, miramos pocas líneas cercanas, descartando mínimo, tasas, límites, etc.
  */
  for (let i = 0; i < lineas.length; i++) {
    ...
    const montosMismaLinea = obtenerMontos(lineas[i]);

    if (montosMismaLinea.length > 0) {
      totalPagar = montosMismaLinea[montosMismaLinea.length - 1];
      break;
    }

    const candidatos = [];

    for (let j = i + 1; j <= i + 5 && j < lineas.length; j++) {
      if (esLineaQueNoSirveParaTotal(lineas[j])) {
        continue;
      }
  ...
  /*
    2) PAGO MÍNIMO
    Buscamos específicamente pago mínimo.
  */
  for (let i = 0; i < lineas.length; i++) {
    ...
    const montos = obtenerMontos(bloque)
      .filter((monto) => monto >= 1000 && (!totalPagar || monto <= totalPagar));
  ...
  */
  if (!pagoMinimo) {
    ...
    const matchMinimo = textoNormalizado.match(
      /pago\s+minimo\s+de[^0-9]{0,40}((?:\d{1,3}(?:\.\d{3})+|\d+),\s*\d{2})/
    );
  ...
  /*
    4) FECHA DE VENCIMIENTO / PAGO
    Buscamos fecha cerca de vencimiento.
    No usamos fechas de consumos ni cuotas como fallback fuerte.
  */
  for (let i = 0; i < lineas.length; i++) {
  ...
  /*
    5) Fallback más inteligente:
    Si no encontró total, buscar una línea exacta TOTAL A PAGAR en texto unido.
  */
  if (!totalPagar) {
    const textoPlano = lineas.join(" ");
    const textoNormalizado = normalizarTexto(textoPlano);
    ...
    if (posicionTotal >= 0) {
      const bloque = textoPlano.slice(posicionTotal, posicionTotal + 300);
      const montos = obtenerMontos(bloque)
        .filter((monto) => monto >= 1000 && monto <= 1000000);

      const candidatos = pagoMinimo
        ? montos.filter((monto) => monto > pagoMinimo)
        : montos;

      if (candidatos.length > 0) {
        totalPagar = Math.max(...candidatos);
      } else if (montos.length > 0) {
        totalPagar = Math.max(...montos);
      }
    }
  }
  ...
  /*
    ...
    Si total y mínimo quedaron iguales, intentamos buscar un total mayor cerca de TOTAL A PAGAR.
  */
  if (totalPagar && pagoMinimo && totalPagar === pagoMinimo) {
    for (let i = 0; i < lineas.length; i++) {
      ...
      const candidatos = obtenerMontos(bloque)
        .filter((monto) => monto > pagoMinimo && monto <= 1000000);
  ...
  /*
    7) Evitar errores obvios:
    Si el pago mínimo quedó mayor que el total, lo limpiamos.
  */
  ...
  /*
    8) Fecha de vencimiento para este resumen.
    Si el PDF no permite detectarla bien, dejamos vacío para corregirla.
    Pero si el nombre o el texto es mayo 2026 y no detectó fecha, usamos 2026-06-01.
  */
  if (!fechaPago) {
