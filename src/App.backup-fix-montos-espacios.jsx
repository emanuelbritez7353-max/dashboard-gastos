...
function detectarResumenPago(textoPDF, nombrePDF) {
  ...
  function obtenerFechas(texto) {
    return String(texto).match(/\b\d{1,2}[\/-]\d{1,2}(?:[\/-]\d{2,4})?\b/g) || [];
  }
  ...
  /*
    1) Detectar TOTAL A PAGAR.
    Tomamos un bloque posterior a "TOTAL A PAGAR" y elegimos el mayor monto razonable.
    Esto evita que tome el pago mínimo por error.
  */
  const posicionTotal = textoNormalizado.indexOf("total a pagar");

  if (posicionTotal >= 0) {
    const bloqueTotal = textoPlano.slice(posicionTotal, posicionTotal + 800);
    const montosTotal = obtenerMontos(bloqueTotal)
      .filter((monto) => monto > 1000 && monto < 10000000);

    if (montosTotal.length > 0) {
      totalPagar = Math.max(...montosTotal);
    }
  }

  /*
    2) Detectar PAGO MÍNIMO.
    Buscamos específicamente frases tipo:
    "pago mínimo de $ 50630,00"
    "pago minimo 50.630,00"
  */
  const matchPagoMinimo = textoNormalizado.match(
    /pago\s+minimo(?:\s+de)?[^0-9]{0,80}(\d{1,3}(?:\.\d{3})+(?:,\d{2})|\d+,\d{2})/
  );

  if (matchPagoMinimo && matchPagoMinimo[1]) {
    pagoMinimo = convertirMonto(matchPagoMinimo[1]);
  }

  /*
    3) Segunda búsqueda de pago mínimo por líneas, si no se detectó.
  */
  if (!pagoMinimo) {
    for (let i = 0; i < lineas.length; i++) {
      const lineaNormalizada = normalizarTexto(lineas[i]);

      if (lineaNormalizada.includes("pago minimo")) {
        const bloque = lineas.slice(i, i + 4).join(" ");
        const montos = obtenerMontos(bloque)
          .filter((monto) => monto > 0 && (!totalPagar || monto <= totalPagar));
  ...
  /*
    4) Detectar fecha de pago / vencimiento.
  */
  for (let i = 0; i < lineas.length; i++) {
    const lineaNormalizada = normalizarTexto(lineas[i]);

    if (
      lineaNormalizada.includes("vencimiento") ||
      lineaNormalizada.includes("fecha de pago") ||
      lineaNormalizada.includes("fecha pago") ||
      lineaNormalizada.includes("pagar hasta")
    ) {
  ...
  */
  ...
  // Si el pago mínimo quedó igual al total, puede estar bien, pero en tarjetas suele ser sospechoso.
  // En tu resumen Visa, el total debería ser mayor que el mínimo.
  if (totalPagar && pagoMinimo && pagoMinimo > totalPagar) {
    pagoMinimo = 0;

    const matchMinimoSeguro = textoNormalizado.match(
      /pago\s+minimo\s+de[^0-9]{0,80}(\d{1,3}(?:\.\d{3})+(?:,\d{2})|\d+,\d{2})/
    );

    if (matchMinimoSeguro && matchMinimoSeguro[1]) {
      const minimoSeguro = convertirMonto(matchMinimoSeguro[1]);

      if (minimoSeguro > 0 && minimoSeguro <= totalPagar) {
        pagoMinimo = minimoSeguro;
      }
    }
  }
  ...
  /*
    6) Fallback especial:
    Si el total quedó igual al pago mínimo, buscamos otro monto cercano a TOTAL A PAGAR
    que sea mayor al mínimo.
  */
  if (totalPagar && pagoMinimo && totalPagar === pagoMinimo) {
    const posicion = textoNormalizado.indexOf("total a pagar");

    if (posicion >= 0) {
      const bloque = textoPlano.slice(posicion, posicion + 1200);
      const candidatos = obtenerMontos(bloque)
        .filter((monto) => monto > pagoMinimo && monto < 10000000);
