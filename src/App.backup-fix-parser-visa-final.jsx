...
function detectarResumenPago(textoPDF, nombrePDF) {
  ...
  function obtenerMontos(texto) {
    const encontrados =
      String(texto).match(
        /(?:\$|ARS)?\s*-?\d[\d.\s]*,\s*\d{1,2}|(?:\$|ARS)?\s*-?\d{1,3}(?:\.\d{3})+/gi
      ) || [];
  ...
  function buscarFecha() {
    ...
    if (primeraFecha) {
      return convertirFecha(primeraFecha[0]);
    }
  ...
  /*
    TOTAL A PAGAR:
    Buscamos la línea que contiene TOTAL A PAGAR y miramos esa línea
    y las líneas cercanas. Elegimos el mayor monto razonable.
    En tu PDF debería encontrar 72.558,37.
  */
  for (let i = 0; i < lineas.length; i++) {
    const l = normalizarTexto(lineas[i]);
  ...
  /*
    PAGO MÍNIMO:
    Primero buscamos la frase más específica:
  ...
  */
  ...
  const matchMinimo = textoNormalizado.match(
    /pago\s+minimo(?:\s+de)?[^0-9]{0,80}(\d[\d.\s]*,\s*\d{1,2})/
  );
  ...
  /*
    Si no lo encontró, busca por líneas cercanas a PAGO MÍNIMO.
  */
  if (!pagoMinimo) {
    for (let i = 0; i < lineas.length; i++) {
      const l = normalizarTexto(lineas[i]);
  ...
  /*
    Corrección especial:
    Si total y mínimo quedaron iguales, intentamos encontrar otro monto mayor
    cerca de TOTAL A PAGAR.
  */
  ...
  /*
    Último intento:
    Si sigue igual o vacío, buscamos todos los montos del PDF y usamos
    como total el mayor monto razonable que no sea límite/tasa.
    Esto ayuda cuando el PDF separa TOTAL A PAGAR del monto.
  */
  if (!totalPagar || (pagoMinimo && totalPagar === pagoMinimo)) {
    const montosGlobales = [];
    ...
    lineas.forEach((linea) => {
      ...
      obtenerMontos(linea).forEach((monto) => {
        if (monto >= 1000 && monto <= 10000000) {
          montosGlobales.push(monto);
        }
      });
    });

    const candidatos = montosGlobales.filter((monto) => !pagoMinimo || monto > pagoMinimo);
