...
function detectarResumenPago(textoPDF, nombrePDF) {
  ...
  function obtenerMontos(linea) {
    ...
    return encontrados
      .map((texto) => ({
        ...
        posicion: linea.indexOf(texto),
      }))
      .filter((item) => item.numero > 0);
  }

  function obtenerFechas(linea) {
    return linea.match(/\b\d{1,2}\/\d{1,2}(?:\/\d{2,4})?\b/g) || [];
  }

  function esLineaDescartable(linea) {
    ...
    return (
      ...
      l.includes("plan v") ||
      l.includes("cancelar en cuotas")
    );
  }

  function buscarMontoEnLineaExacta(indice) {
    const linea = lineas[indice];

    if (!linea || esLineaDescartable(linea)) {
      return 0;
    }
  ...
  function buscarMontoLineasSiguientes(indice, cantidadLineas = 4) {
    for (let i = indice + 1; i <= indice + cantidadLineas && i < lineas.length; i++) {
      const linea = lineas[i];

      if (esLineaDescartable(linea)) {
        continue;
      }
  ...
  function buscarMontoDespuesDePalabra(indice, palabras) {
    const linea = lineas[indice];
    ...
    let posicionPalabra = -1;

    palabras.forEach((palabra) => {
      const pos = lineaLower.indexOf(palabra);
      if (pos >= 0 && (posicionPalabra === -1 || pos < posicionPalabra)) {
        posicionPalabra = pos;
      }
    });
    ...
    const montosDespues = montos.filter((monto) => monto.posicion >= posicionPalabra);
  ...
  function buscarFechaCerca(indice, cantidadLineas = 4) {
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    const esTotalAPagar =
      ...
      lineaLower.includes("total pagar") ||
    ...
    const esFechaPago =
      ...
      lineaLower.includes("fecha pago") ||
    ...
    if (esFechaPago && !fechaPago) {
      if (fechas.length > 0) {
        ...
        fechaPago = buscarFechaCerca(i, 4);
      }
    }

    if (esTotalAPagar && !totalPagar) {
      totalPagar = buscarMontoEnLineaExacta(i);

      if (!totalPagar) {
        totalPagar = buscarMontoLineasSiguientes(i, 4);
      }
    }

    if (esPagoMinimo && !pagoMinimo) {
      pagoMinimo = buscarMontoDespuesDePalabra(i, [
        "pago mínimo",
        "pago minimo",
      ]);

      if (!pagoMinimo) {
        pagoMinimo = buscarMontoLineasSiguientes(i, 3);
      }
    }
  }
  ...
  /*
    ...
    El pago mínimo nunca debería ser mayor que el total a pagar.
    Si pasa eso, es porque el PDF mezcló otro monto grande del resumen.
  */
  if (totalPagar && pagoMinimo && pagoMinimo > totalPagar) {
    ...
    for (let i = 0; i < lineas.length; i++) {
      ...
      if (
        ...
        const montos = obtenerMontos(lineas[i])
