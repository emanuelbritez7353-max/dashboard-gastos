...
function detectarResumenPago(textoPDF, nombrePDF) {
  ...
  function buscarMontoCerca(indice, cantidadLineas = 4) {
    const montos = [];

    for (let i = indice; i <= indice + cantidadLineas && i < lineas.length; i++) {
      obtenerMontos(lineas[i]).forEach((monto) => {
        const numero = convertirMonto(monto);
        if (numero > 0) {
          montos.push(numero);
        }
      });
  ...
  lineas.forEach((linea, indice) => {
    ...
    const esLineaTotal =
      ...
      lineaLower.includes("total tarjeta") ||
      lineaLower.includes("total actual");
    ...
    if (esLineaFechaPago && !fechaPago) {
      if (fechas.length > 0) {
        fechaPago = convertirFecha(fechas[fechas.length - 1]);
      } else {
        fechaPago = buscarFechaCerca(indice);
      }
    }
    ...
    if (esLineaTotal) {
      if (montos.length > 0) {
        ...
        totalPagar = buscarMontoCerca(indice);
      }
    }
    ...
    if (esLineaMinimo) {
      if (montos.length > 0) {
        ...
        pagoMinimo = buscarMontoCerca(indice);
      }
    }
  });

  /*
    Fallback especial para resúmenes donde el texto sale separado.
    Busca bloques tipo:
    TOTAL A PAGAR
    72.558,37
  */
  if (!totalPagar) {
    for (let i = 0; i < lineas.length; i++) {
      ...
      if (lineaLower.includes("total a pagar")) {
        totalPagar = buscarMontoCerca(i, 6);
        break;
      }
    }
  }
  ...
  if (!pagoMinimo) {
    for (let i = 0; i < lineas.length; i++) {
      ...
      if (
        ...
        pagoMinimo = buscarMontoCerca(i, 6);
        break;
      }
    }
  }
  ...
  if (!fechaPago) {
    for (let i = 0; i < lineas.length; i++) {
      ...
      if (
        ...
        fechaPago = buscarFechaCerca(i, 6);
        break;
      }
    }
  }
